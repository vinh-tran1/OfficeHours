...
const ClassCard = ({ cls, events, ta }) => {
    return (
        ...
        >
            ...
            <Text fontSize="l" fontWeight="semibold" color="gray">{cls.name}</Text>
            <Text fontSize="l" fontWeight="semibold" color="gray">Location: {cls.room.abbr}</Text>

            <Divider color="black" mb={3} mt={3} />

            {/* Professor OH Info */}
            <Flex justify="space-between" align="center">
                <Text fontSize="xl" fontWeight="semibold" color="black">My Office Hours</Text>
                <IconButton 
                    icon={<FaEdit size="20" color="black" />}
                    variant="ghost"
                />
            ...
            <Flex justify="space-between" align="center">
                <Text fontSize="xl" fontWeight="semibold" color="black">TAs/ULAs</Text>
                <IconButton 
                    icon={<FaEdit size="20" color="black" />}
                    variant="ghost"
                />
            ...
            {ta.map((assistant, index) => (
              <Box key={index}>
                <Text fontSize="l" fontWeight="semibold" color="black">{assistant.name}</Text>
                <Text fontSize="l" fontWeight="semibold" color="gray">Office hours: {assistant.hours}</Text>
              </Box>
            ))}
